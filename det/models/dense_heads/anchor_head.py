...
from det.core import (multi_apply, unmap, images_to_levels, anchor_inside_flags,
                      build_assigner, build_sampler, build_bbox_coder, build_anchor_generator, multiclass_nms)
...
class AnchorHead(BaseDenseHead):
    ...
    def forward_single(self, x):
        ...
        Returns:
            tuple:
                ...
                the channels number is num_anchor * num_classes.
    ...
    def forward(self, feats):
        """
        Forward features from network.
        Args:
            feats: (tuple[Tensor]): Features with multi-level from network, each is 4-D tensor.

        Returns:
            tuple: A tuple of classification scores and bbox prediction.
                cls_scores (list[Tensor]): cls scores for all scale levels, each is a 4-D tensor,
                the channels number is num_anchors * num_classes.
                bbox_preds (list[Tensor]): box energies / deltas for all scale levels, each is
                4-D tensor, the channels number is num_anchors *4.
        """
    ...
    def _get_target_single(self,
        ...
        Args:
            ...
            which are concatenated into a single tensor of shape (num,anchors,).
        ...
        labels = anchors.new_full((num_valid_anchors,), self.background_label,
                                  dtype=torch.long)  # (size, fill_value,dtype)
    ...
    def get_targets(self,
        ...
        num_total_pos = sum([max(inds.numel(), 1) for inds in pos_inds_list])  # numel: return total number of elements
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels)  # (N,H,W,C) -> (-1,num_classes)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            scale_fator = img_metas[img_id]['scale_factor']
            proposals = self._get_bboxes_single(cls_score_list, bbox_pred_list,
                                                mlvl_anchors, img_shape, scale_fator,
                                                cfg, rescale)
    ...
    def _get_bboxes_single(self,
        ...
        if self.use_sigmoid_cls:
            ...
            # remind that FG labels is [0,num_class-1] BG cat_id: num_class
