...
from det.core import PolygonMasks
from det.core.evaluation.bbox_overlaps import bbox_overlaps
...
class Resize(object):
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.img_scale = [img_scale]
            assert mtcv.is_list_of(self.img_scale, tuple)
    ...
    def random_select(img_scales):
        ...
        assert mtcv.is_list_of(img_scales, tuple)
    ...
    def random_sample(img_scales):
        ...
        assert mtcv.is_list_of(img_scales, tuple) and len(img_scales) == 2
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = mtcv.imrescale(
                    results[key], results['scale'], return_scale=True)
                # the w_scale and h_scale has minor difference
                # a real fix should be done in the mtcv.imrescale in the future
                new_h, new_w = img.shape[:2]
                h, w = results[key].shape[:2]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                img, w_scale, h_scale = mtcv.imresize(
                    results[key], results['scale'], return_scale=True)
            results[key] = img
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = mtcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
            else:
                gt_seg = mtcv.imresize(
                    results[key], results['scale'], interpolation='nearest')
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.size is not None:
                padded_img = mtcv.impad(
                    results[key], shape=self.size, pad_val=self.pad_val)
            elif self.size_divisor is not None:
                padded_img = mtcv.impad_to_multiple(
...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            results[key] = mtcv.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
